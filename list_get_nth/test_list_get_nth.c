...
int get_nth(int value, struct node *head);

static void print_list(struct node *head);
static void check_list_unchanged(char *name, struct node *head, int get_nth, struct node **original_node, int *original_data);
...
int main(int argc, char *argv[]) {
    ...
    int return_value = get_nth(value, head);

    // If you're getting an error here,
    // you have returned an uninitialized value
    printf("get_nth returned: %d\n", return_value);
...
static void check_list_unchanged(char *name, struct node *head, int get_nth, struct node **original_nodes, int *original_data) {
    struct node *p = head;
    for (int i = 0; i < get_nth; i++) {
