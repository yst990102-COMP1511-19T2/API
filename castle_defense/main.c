...
void do_apply_effect(Realm realm, char *command) {
    ...
    if (num_scanned == 3) {
        ...
        } else if (effect_type_char == EFFECT_BOUNCE_CHAR) {
            effect_type = EFFECT_BOUNCE;
        }
        ...
        if (effect_type == INVALID_INPUT) {
            ...
            printf("%d tower(s) have a new effect!\n", matches);
        }
    } else {
        printf("Invalid command!\n");
    }
...
void do_new_tower(Realm realm, char *command) {
    char ignored_char;
    char prev_name[MAX_INVALID_NAME_ARRAY_SIZE];
    char name[MAX_INVALID_NAME_ARRAY_SIZE];
    int power, uses;

    int num_scanned = sscanf(
        command, "%c %s %s %d %d",
        &ignored_char, prev_name, name,
        &power, &uses
    );
    
    if (!valid_name(prev_name)) {
        printf(
    ...
    if (num_scanned == 5) {
        int result = new_tower(realm, prev_name, name, power, uses);
        if (result == ERROR_NO_LOCATION) {
            printf("Could not find tower %s!\n", prev_name);
        } else if (result == ERROR_INVALID_STAT) {
