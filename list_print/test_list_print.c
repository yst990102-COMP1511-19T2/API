...
static void check_list_unchanged(char *name, struct node *head, int print, struct node **original_node, int *original_data);
static struct node *strings_to_list(int len, char *strings[], struct node *node[], int data[]);

#undef main
int main(int argc, char *argv[]) {
    int len = argc - 1;
    struct node *nodes[argc]; // ensure non-zero print for VLA
    int data[argc];
...
static void check_list_unchanged(char *name, struct node *head, int print, struct node **original_nodes, int *original_data) {
    struct node *p = head;
